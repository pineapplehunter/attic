...
fn test_generate_key() {
    ...
    // re-import keypair
    let import = NixKeypair::import(&export_priv).expect("Could not re-import generated key");

    assert_eq!(keypair.name, import.name);
    assert_eq!(keypair.keypair, import.keypair);

    // re-import public key
    let import_pub = NixPublicKey::import(&export_pub).expect("Could not re-import public key");
    ...
    let import_pub2 = NixPublicKey::import(&export_pub2).expect("Could not re-import public key");
...
fn test_import_public_key() {
    ...
    let import = NixPublicKey::import(cache_nixos_org).expect("Could not import public key");
